...
export default function DevolverPage() {
    ...
    // Busca livros alugados pelo usuário logado
    const fetchLivrosAlugados = async () => {
        setLoading(true);
        setErro("");
        try {
            ...
            const res = await axios.get(
                `http://localhost:5000/api/users/${usuario_id}/alugados`
            );
            setLivros(res.data);
        } catch (e) {
            setErro("Erro ao buscar livros alugados.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDevolver = async (livro: Livro) => {
        setDevolvendo(true);
        setMensagem("");
        try {
            const usuario_id = localStorage.getItem("usuario");
            ...
            const res = await axios.post(
                `http://localhost:5000/api/books/${livro.id}/return`,
                { user_id: usuario_id }
            );
            setMensagem(res.data.message || "Livro devolvido com sucesso!");
            await fetchLivrosAlugados();
            setTimeout(() => {
                handleFecharModal();
            }, 1200);
        } catch (e: any) {
            setMensagem(
                e.response?.data?.error ||
                    "Erro ao tentar devolver o livro. Tente novamente."
            );
        } finally {
            setDevolvendo(false);
        }
                            ...
                            {livros.map((livro) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 flex flex-col items-center">
                                        ...
                                        {livro.data_devolucao_estimada && (
                                            <span className="mt-2 px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">
                                                Devolver até:{" "}
                                                {new Date(
                                                    livro.data_devolucao_estimada
                                    ...
                                    <button
                                        className="mt-4 w-full py-2 rounded-lg font-bold transition bg-blue-600 text-white hover:bg-blue-700"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleAbrirModal(livro);
                                        }}
                                    >
                                        Devolver
                                    </button>
                        ...
                        {livroSelecionado && (
                            <div>
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    {livroSelecionado.data_devolucao_estimada && (
                                        <div className="mt-4 px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">
                                            Devolver até:{" "}
                                            {new Date(
                                                livroSelecionado.data_devolucao_estimada
                                            ).toLocaleDateString("pt-BR")}
                                    ...
                                    <button
                                        className="mt-6 w-full py-2 rounded-lg font-bold transition bg-blue-600 text-white hover:bg-blue-700"
                                        disabled={devolvendo}
                                        onClick={() =>
                                            handleDevolver(livroSelecionado)
                                        }
                                    >
                                        {devolvendo
                                            ? "Devolvendo..."
